...
<tbody>
<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr><td><?php echo ($vo["foreign_trade"]); ?></td><td><?php echo ($vo["company_name"]); ?></td><td><?php echo ($vo["bank_name"]); ?></td><td><?php echo ($vo["pay_dd"]); ?></td><td><?php echo ($vo["bank_cust"]); ?></td><td><?php echo ($vo["country"]); ?></td><td><?php echo ($vo["CUR_ID"]); ?></td><td><?php echo ($vo["amount"]); ?></td></tr><?php endforeach; endif; else: echo "" ;endif; ?>
</tbody>
</table>
<br /><br /><br />
<form action="getAccountData" method="post">
<input type="hidden" name="id" value="<?php echo ($vo["id"]); ?>" />
请选择您的名字:<select name="sal_name" id="btext"><option >-------</option><?php if(is_array($data_acc)): foreach($data_acc as $key=>$vo_acc): ?><option value="<?php echo ($vo_acc["sal_name"]); ?>"><?php echo ($vo_acc["sal_name"]); ?></option><?php endforeach; endif; ?></select><br />
<fonthidd><p>如果此笔到款是有多个业务员合并的请输入</p>
其他业务员名字:<input name="sal_name2" type="text" placeholder="多个名字中间用空格分隔"></fonthidd><br /><br />

<input type="submit" value="认领" />&nbsp;<button type="button" onclick="javascript:history.back(-1)">返回</button>
<br /><br />
<b>如果没有您的名字，请更新台账</b>
</form>
