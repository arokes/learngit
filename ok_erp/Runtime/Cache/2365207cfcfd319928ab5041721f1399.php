...
<table id="table-5">
<thead>
<th>单据类别</th><th>订单号</th><th>品号</th><th width="250px">品名</th><th>客户编号</th><th>单号</th><th>单据日期</th><th>入库数量</th><th>出库数量</th><th>出库金额</th><th>结存数量</th>
</thead>
<tbody>
<?php if(is_array($vo_1)): $i = 0; $__LIST__ = $vo_1;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr><td><?php echo (iconv("GBK","utf-8",$vo["ps_id"])); ?></td><td><?php echo ($vo["bat_no"]); ?></td><td><?php echo ($vo["prd_no"]); ?></td><td width="250px"><?php echo (iconv("GBK","utf-8",$vo["prd_name"])); ?></td><td><?php echo ($vo["prd_mark"]); ?></td><td><?php echo ($vo["ps_no"]); ?></td><td><?php echo ($vo["eff_dd"]); ?></td><td><?php echo (round($vo["pc_qty"],0)); ?></td><td><?php echo (round($vo["sa_qty"],0)); ?></td><td><?php echo (round($vo["sa_amt"],2)); ?></td><td><?php $stock=$stock+$vo['pc_qty']-$vo['sa_qty']; echo ($stock); ?></td></tr><?php endforeach; endif; else: echo "" ;endif; ?>
</tbody>
