...
收汇确认业务员或跟单:<b><?php echo ($sal_name); ?></b>
<?php if(empty($data_acc)): ?>没有可以选择的订单!<?php else: ?>
...
<tbody>
<?php if(is_array($data_acc)): $i = 0; $__LIST__ = $data_acc;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr name="<?php echo ($vo["so_no"]); ?>" class="screen" ><td><input type="checkbox" name="so_id[]" value="<?php echo ($vo["so_id"]); ?>" /></td><td><a target="_blank" href="../Account/SelectAccountDetail?so_id=<?php echo ($vo["so_id"]); ?>"><?php echo ($vo["so_no"]); ?></td><td><?php echo ($vo["cust_name"]); ?></td><td><?php echo ($vo["sal_name"]); ?></td><td><?php echo ($vo["balance_amount"]); ?></td><td><?php echo ($vo["receivable"]); ?></td><td><?php echo ($vo["amount_apart"]); ?></td><td><?php echo ($vo["brokerage"]); ?></td><td><?php echo ($vo["CUR_ID"]); ?></td><td><?php echo ($vo["HONGKONG_PI"]); ?></td><td><?php echo ($vo["CHINA_PI"]); ?></td></tr><?php endforeach; endif; else: echo "" ;endif; ?>
</tbody>
</table><?php endif; ?>
<input type="submit" value="选择完毕" />&nbsp;<a href="<?php echo ($url_back); ?>">返回</a><br /><br />
