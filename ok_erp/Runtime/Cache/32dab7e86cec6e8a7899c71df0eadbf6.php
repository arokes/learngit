...
<form method="post" action="ModifyAccountResult" id="contact-form">
<input type="hidden" readonly name="so_id" value=<?php echo ($account["so_id"]); ?> />
<label>客户名称</label>
<input type="text" readonly name="cust_name" value="<?php echo ($account["cust_name"]); ?>"  />
<label>国家</label>
<input type="text" readonly name="country" value="<?php echo ($account["country"]); ?>"  />
<label>订单号</label>
<input type="text" readonly name="so_no" value="<?php echo ($account["so_no"]); ?>" />
<label>币别</label>
<select name="CUR_ID">
<option value="USD" <?php echo ($account['CUR_ID']=='USD'?'selected="selected"':''); ?>>USD</option>
<option value="RMB" <?php echo ($account['CUR_ID']=='RMB'?'selected="selected"':''); ?>>RMB</option>
<option value="EUR" <?php echo ($account['CUR_ID']=='EUR'?'selected="selected"':''); ?>>EUR</option>
</select>
<label>与香港公司PI金额</label>
<input type="text" name="HONGKONG_PI" value="<?php echo ($account["HONGKONG_PI"]); ?>" />
<label>与国内公司PI金额</label>
<input type="text" name="CHINA_PI" value="<?php echo ($account["CHINA_PI"]); ?>" />
<label>预计出货时间</label>
<input type="text" name="expect_sale" value="<?php echo ($account["expect_sale"]); ?>" onClick="laydate()" onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-9--]+/,'');}).call(this)" onblur="this.v();" />
<label>应收款</label>
<input type="text" name="receivable" value="<?php echo ($account["receivable"]); ?>"  />
<label>账期</label>
<input type="text" name="recevable_dd" value="<?php echo ($account["recevable_dd"]); ?>" onClick="laydate()" onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-9--]+/,'');}).call(this)" onblur="this.v();" />
<label>应收款日期</label>
<input type="text" name="recevable_dd" value="<?php echo ($account["recevable_dd"]); ?>" onClick="laydate()" onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-9--]+/,'');}).call(this)" onblur="this.v();" />
<label>业务员</label>
<input type="text" readonly name="sal_name" value="<?php echo ($account["sal_name"]); ?>" />
<label>备注</label>
<input type="text" name="rem" value="<?php echo ($account["rem"]); ?>" />

<button id="submit-button" onclick="javascript:history.back(-1);">返回</button><input type="submit" value="修改" id="submit-button" />
<p></p><p></p>
