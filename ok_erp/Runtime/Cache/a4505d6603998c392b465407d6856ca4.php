...
<font>修改认领金额</font>
<table id="table-5">
<thead><th>编号</th><th>销售方式</th><th>公司</th><th>银行</th><th>到款日期</th><th>汇款人名字</th><th>汇款人国家</th><th>币别</th><th>到账金额</th><th>业务员</th><th>开票员</th></thead>
<tbody>
...
<br />
<form method="post" action="__URL__/changeClaimResult" >
<table id="table-5">
<thead>
...
<input type="hidden" value="<?php echo ($data["0"]["amount"]); ?>" name="amount_sum" />
<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
<input type="hidden" value="<?php echo ($vo["itm"]); ?>" name="itm[]" />
<td><input id="cst" type="text" name="amount_apart[]" value="<?php echo ($vo["amount_apart"]); ?>" /></td>
<td><input id="cst" type="text" name="brokerage[]" value="<?php echo ($vo["brokerage"]); ?>" /></td>
<td><input type="hidden" id="os_no" type="text" name="so_no[]" value="<?php echo (htmlspecialchars_decode($vo["so_no"])); ?>" /><?php echo (htmlspecialchars_decode($vo["so_no"])); ?></td>
<td><input type="hidden" id="cst" type="text" name="so_pi[]" value="<?php echo ($vo["so_pi"]); ?>" /><?php echo ($vo["so_pi"]); ?></td>
<td><input type="radio" value="是" <?php echo ($vo['is_declare']=='是'?"checked=\"checked\"":''); ?> name="is_declare<?php echo ($i); ?>" />是<input type="radio" value="否"   <?php echo ($vo['is_declare']=='否'?"checked=\"checked\"":''); ?> name="is_declare<?php echo ($i); ?>" />否</td>
<td><input type="hidden" id="cust" type="text" value="<?php echo (htmlspecialchars_decode($vo["contract_cust"])); ?>"  name="contract_cust[]" /><?php echo (htmlspecialchars_decode($vo["contract_cust"])); ?></td>
<td><input id="cust" type="text" value="<?php echo (htmlspecialchars_decode($vo["lz_cust"])); ?>" name="lz_cust[]" /></td>
<td><input id="cust" type="text" value="<?php echo (htmlspecialchars_decode($vo["connect_cust"])); ?>" name="connect_cust[]" /></td>
<td><input type="text" value="<?php echo (htmlspecialchars_decode($vo["debit"])); ?>" name="debit[]" /></td>
<td><input type="text" value="<?php echo (htmlspecialchars_decode($vo["rem"])); ?>" name="rem[]"  /></td>
</tr><?php endforeach; endif; else: echo "" ;endif; ?>
</tbody>
</table>
<br />
<input type="submit" value="保存" /><button type="button" onclick="javascript:history.back(-1)">返回</button></form>
