...
<?php if(empty($storagegoods_data)): ?>没有需要核对的库存商品<?php else: ?>
<h3>核对库存商品</h3>
截至9月17日系统库存
<table id="table-4">
<thead>
<th>公司名称</th><th>订单号</th><th width="500px">品名</th><th>库存数量</th><th></th>
</thead>
<tbody>
<?php if(is_array($storagegoods_data)): $i = 0; $__LIST__ = $storagegoods_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr><td><?php echo ($vo["company_name"]); ?></td><td><?php echo ($vo["so_no"]); ?></td><td width="500px"><?php echo ($vo["prd_name"]); ?></td><td><?php echo ($vo["qty"]); ?></td><td><a href="CheckStorageGoodsDetail?so_no=<?php echo ($vo["so_no"]); ?>">核对这个批号</a></td></tr><?php endforeach; endif; else: echo "" ;endif; ?>
</tbody>
</table><?php endif; ?>
