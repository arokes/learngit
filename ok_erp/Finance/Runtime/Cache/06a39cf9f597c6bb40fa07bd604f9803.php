...
<form method="POST" action="__URL__/DeletedExchangeResult" id="contact-form">
<label>客户名称:</label>
<input type="text" name="bank_cust" />
<label>公司:</label>
<select name="company_id">
<option value=''>---------</option>
<?php if(is_array($listcompany)): $i = 0; $__LIST__ = $listcompany;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$voc): $mod = ($i % 2 );++$i;?><option value="<?php echo ($voc["company_id"]); ?>" ><?php echo ($voc["company_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select>
<label>银行:</label>
<select name="bank_id">
<option value=''>---------</option>
<?php if(is_array($listbank)): $i = 0; $__LIST__ = $listbank;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vob): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vob["bank_id"]); ?>" ><?php echo ($vob["bank_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select>
<label>金额:</label>
