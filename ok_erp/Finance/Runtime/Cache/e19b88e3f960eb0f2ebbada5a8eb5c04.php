...
<form method="POST" action="ModifyBillingCompanyBasicResult" id="contact-form" >
<label>开票公司名称:</label>
<select name="mekacompany">
<?php if(is_array($company_data)): $i = 0; $__LIST__ = $company_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["company_name"]); ?>"><?php echo ($vo["company_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select>
<label>购方公司名称:</label>
<input type="text" name="Gfmc" />
<label>购方公司税号:</label>
<input type="text" name="Gfsh">

<label>购方银行账号:</label>

<input type="text" name="Gfyhzh">
<label<input type="text" name="Gfdzdh">
...
<input type="text" name="Skr">
<label>备注:</label>
<input type="text" name="Bz">
<input type="submit" value="新增" id="submit-button"><button type="button" class="tolink" url="FormatBillingDetailList" id="submit-button">返回</button>
<p></p>
</form>
