...
<center>
<?php if(is_array($billdetail_naked_result)): $i = 0; $__LIST__ = $billdetail_naked_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><font><?php echo ($key); ?>--裸灯缴库</font>
<table id="table-2">
<thead>
<th>批号</th><th>成品品号</th><th>裸灯品号</th><th width="300px">裸灯品名</th><th>缴库数量</th>
</thead>
<?php if(is_array($vo)): $i = 0; $__LIST__ = $vo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo_1): $mod = ($i % 2 );++$i;?><tbody>
<tr><td width="100px"><?php echo (iconv("GBK","UTF-8",$vo_1["bat_no"])); ?></td><td><?php echo (iconv("GBK","UTF-8",$vo_1["mrp_no_so"])); ?></td><td><?php echo ($vo_1["prd_no"]); ?></td><td width="300px"><?php echo (iconv("GBK","UTF-8",$vo_1["prd_name"])); ?></td><td><?php echo (round($vo_1["qty"],0)); ?></td>
