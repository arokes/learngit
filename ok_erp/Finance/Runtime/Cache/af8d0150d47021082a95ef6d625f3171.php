...
<center>
<?php if(is_array($billdetail_so_result)): $i = 0; $__LIST__ = $billdetail_so_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><font><?php echo ($key); ?>--销售订单明细</font>
<table id="table-2">
<thead>
<th width="100px">客户</th><th>业务员</th><th>批号</th><th>品号</th><th width="300px">品名</th><th>订单数量</th><th>退回数量</th>
</thead>
<?php if(is_array($vo)): $i = 0; $__LIST__ = $vo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo_1): $mod = ($i % 2 );++$i;?><tbody>
<tr><td width="100px"><?php echo (iconv("GBK","UTF-8",$vo_1["cus_name"])); ?></td><td><?php echo (iconv("GBK","UTF-8",$vo_1["sal_name"])); ?></td><td><?php echo (iconv("GBK","UTF-8",$vo_1["bat_no"])); ?></td><td><?php echo ($vo_1["prd_no"]); ?></td><td width="300px"><?php echo (iconv("GBK","UTF-8",$vo_1["prd_name"])); ?></td><td><?php echo (round($vo_1["qty"],0)); ?></td><td><?php echo (round($vo_1["qty_pre"],0)); ?></td></tr>
