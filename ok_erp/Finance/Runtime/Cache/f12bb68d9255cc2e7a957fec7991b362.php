...
<h3>查询开票资料结果</h3>
<table id="table-4">
<thead><th width="220px">合同协议号</th><th>业务员</th><th>报关公司名</th><th>预录入单</th><th>内销合同</th><th>清关资料</th><th>发货清单</th><th>上传时间</th><th>修改时间</th><th>备注</th><th></th>
</thead>
<tbody>
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr><td width="220px"><?php echo ($vo["contract_no"]); ?></td><td><?php echo ($vo["sale_name"]); ?></td><td><?php echo ($vo["company_name"]); ?></td><td><?php if(empty($vo["prerecorded_file"])): else: ?><a href="__URL__/DownloadBillingData?filename=<?php echo ($vo["prerecorded_file"]); ?>&showname=<?php echo ($vo["contract_no"]); ?>预录入单">预录入单</a><?php endif; ?></td><td><?php if(empty($vo["contract_file"])): else: ?><a href="__URL__/DownloadBillingData?filename=<?php echo ($vo["contract_file"]); ?>&showname=<?php echo ($vo["contract_no"]); ?>内销合同">内销合同</a><?php endif; ?></td><td><?php if(empty($vo["clearance_file"])): else: ?><a href="__URL__/DownloadBillingData?filename=<?php echo ($vo["clearance_file"]); ?>&showname=<?php echo ($vo["contract_no"]); ?>清关资料">清关资料</a><?php endif; ?></td>
<td><?php if(empty($vo["sendgoods_file"])): else: ?><a href="__URL__/DownloadBillingData?filename=<?php echo ($vo["sendgoods_file"]); ?>&showname=<?php echo ($vo["contract_no"]); ?>发货清单">发货清单</a><?php endif; ?></td><td><?php echo ($vo["upload_date"]); ?></td><td><?php echo ($vo["change_date"]); ?></td><td><?php echo ($vo["rem"]); ?></td><td><a href="__URL__/DownloadAllBillingData?id=<?php echo ($vo["ID"]); ?>">下载全部</a></td>
